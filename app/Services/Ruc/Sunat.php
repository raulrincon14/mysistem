...
namespace App\Services\Ruc;

use App\Services\Helpers\HtmlParser;
use App\Services\Helpers\Http\ClientInterface;
use App\Services\Helpers\Http\ContextClient;
...
class Sunat
{
    ...
    public function get($ruc)
    {
        ...
        $persona=substr($ruc, 0, 2);
        if ($persona == '10') {
          return $this->getCompany1($dic);
        }else {
    ...
    private function getCompany1(array $items)
    {
        $cp = $this->getHeadCompany($items);
        $cp->sistEmsion = $items['Sistema de Emisión de Comprobante:'];
        $cp->sistContabilidad = $items['Sistema de Contabilidad:'];
    ...
    private function fixDirection(Company $company)
    {
        ...
        $pieces = explode(' ', $items[$numare]);
        list($len, $value) = $this->getDepartment(end($pieces));
        $company->departamento = $value;
        $company->provincia = $items[$numare+1];
        $company->distrito = $items[$numare+2];
        array_splice($pieces, -1 * $len);
        $company->direccion = implode(array_slice($items, 0, $numare)).' '.join(' ', $pieces);
        }else{
          for ($i=0;$i<=2;$i++)
          {
              $items[$i] = trim($items[$i]);
